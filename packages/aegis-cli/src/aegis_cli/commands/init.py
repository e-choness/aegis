...
# Run `aegis serve --config aegis.yaml --no-auth` to start immediately.
# Run `aegis config validate aegis.yaml` to validate this file.

# ── Providers ─────────────────────────────────────────────────────────────
# A `fake` provider is wired by default so the server starts without API keys.
# Replace with `openai_compatible` or `anthropic` for real model calls.
